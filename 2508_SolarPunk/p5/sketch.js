...
p.createSoundObject = function() {
  sound_objects.push(new SoundObject(p.mouseX, p.mouseY, buffer_index));
}

p.destroySoundObject = function() {
  for (let i = sound_objects.length - 1; i >= 0; i--)
  {
    const so = sound_objects[i];
    let prob = Math.pow(SURVIVAL_PROB, so.age);
    if(so.age >= MAX_AGE || Math.random() > prob)
    {
        sound_objects.splice(i, 1);
        p.print("if");
    }
    else
    {
...
class SoundObject {
  ...
  display() {
    ...
    p.fill(0);
    p.textSize(20);
    p.text(this.id+" "+this.age, this.x, this.y);
  }
